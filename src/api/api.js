...
class JoblyApi {
  ...
  /** Get details on a company by handle WITH company jobs */

  static async getCompanyWithJobs(handle) {
    if(handle) {
      const jobs = mocks.jobs.filter(c => {
        return c.companyHandle.toLocaleLowerCase().includes(handle.toLocaleLowerCase())
      });

      const currcompany = mocks.companies.filter(c => {
        return c.handle.toLocaleLowerCase().includes(handle.toLocaleLowerCase())
      })
      
      return {jobs, currcompany};
    }
